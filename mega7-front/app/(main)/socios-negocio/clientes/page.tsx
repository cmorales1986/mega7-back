...
import { Plus, RefreshCcw, Pencil, Trash2, Power, Eye, Users2, Search } from "lucide-react";
...
export default function SociosClientesPage() {
  ...
  async function loadData() {
    ...
    try {
      ...
      Swal.fire("Error", err?.response?.data ?? "No se pudo cargar clientes", "error");
    }
    setLoading(false);
  }
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      const hay = `${r.code} ${r.razonSocial} ${r.ruc} ${r.telefono ?? ""} ${r.email ?? ""} ${
        r.ciudad ?? ""
      }`.toLowerCase();
  ...
  const stats = useMemo(() => {
    ...
    const withCredit = rows.filter((x) => !!x.creditTermId || (x.creditLimit ?? 0) > 0).length;
    const withInstallments = rows.filter((x) => !!x.allowInstallments).length;
    return { total, active, inactive, withCredit, withInstallments };
  }, [rows]);
  ...
  const validate = (): string | null => {
    ...
    if (!ruc) return "RUC / Cédula es obligatorio.";
    ...
    if (allow) {
      if (!max || max < 2) return "Máx. cuotas debe ser >= 2 si 'Permitir cuotas' está activo.";
        ...
        return (
          <div className="w-full h-full flex items-center justify-center gap-2">
            <Button
              ...
              onClick={() => router.push(`/socios-negocio/clientes/${row.id}`)}
  ...
  return (
    <PageShell
      ...
      title="Clientes"
      subtitle="Socios de negocio (tipo Cliente)"
      chips={
        <>
          <Chip tone="neutral">Total: {stats.total}</Chip>
          <Chip tone="ok">Activos: {stats.active}</Chip>
          <Chip tone="neutral">Inactivos: {stats.inactive}</Chip>
          <Chip tone="info">Con crédito: {stats.withCredit}</Chip>
          <Chip tone="warn">Con cuotas: {stats.withInstallments}</Chip>
        </>
      }
      right={
        <>
          ...
          <div className="space-y-5">
            ...
            <div className="border rounded-xl p-4 bg-gray-50 space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-gray-500">Aplica a condiciones de pago del cliente</span>
              </div>
