...
export default function ProveedorDetailPage() {
  ...
  async function saveSucursal() {
    ...
    try {
      if (!editing) {
        ...
        await api.put(`/socionegociosucursales/${editing.id}`, payload);
  ...
  const columns: GridColDef[] = [
    ...
    { field: "direccion", headerName: "Dirección", width: 280 },
  ...
  return (
    <PageShell
      ...
      title={loadingSocio ? "Proveedor" : socio?.razonSocial ?? "Proveedor"}
      subtitle={socio ? `${socio.code} • RUC: ${socio.ruc}` : "Detalle del proveedor"}
      chips={
        <>
    ...
    >
      ...
      <div className="bg-white rounded-xl border shadow p-4">
        ...
        <div className="mt-4 grid grid-cols-12 gap-4">
          <div className="col-span-12 md:col-span-4 rounded-xl border p-4 bg-slate-50">
            <div className="flex items-center gap-2 text-xs text-slate-500">
              <User2 className="h-4 w-4" /> Contacto
            </div>
            <div className="mt-1 font-medium text-slate-900">{socio?.contacto || "-"}</div>
          </div>
          ...
          <div className="col-span-12 md:col-span-4 rounded-xl border p-4 bg-slate-50">
            ...
            <div className="mt-1 font-medium text-slate-900">{socio?.telefono || "-"}</div>
          </div>
          ...
          <div className="col-span-12 md:col-span-4 rounded-xl border p-4 bg-slate-50">
            ...
            <div className="mt-1 font-medium text-slate-900">{socio?.email || "-"}</div>
          </div>

          <div className="col-span-12 md:col-span-8 rounded-xl border p-4 bg-slate-50">
            <div className="flex items-center gap-2 text-xs text-slate-500">
              <MapPin className="h-4 w-4" /> Dirección
            </div>
            <div className="mt-1 font-medium text-slate-900">{socio?.direccion || "-"}</div>
          </div>

          <div className="col-span-12 md:col-span-4 rounded-xl border p-4 bg-slate-50">
            <div className="text-xs text-slate-500">Ciudad</div>
            <div className="mt-1 font-medium text-slate-900">{socio?.ciudad || "-"}</div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border shadow p-4 space-y-4">
        <SectionHeader
          ...
          subtitle="Filtrá por nombre, ciudad, dirección, teléfono o contacto"
        />
        <Input
